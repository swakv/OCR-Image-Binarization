...
def fix_brightness(img):
    ...
    if avg_maxval>35 and avg_maxval<45:
        norm_img2 = cv2.normalize(img, None, alpha=0, beta=0.6, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        ...
        norm_img2 = cv2.normalize(img, None, alpha=0, beta=0.65, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        norm_img2 = np.clip(norm_img2, 0, 1)
        norm_img2 = (255*norm_img2).astype(np.uint8)
        img = norm_img2

    elif avg_maxval > 70:
        norm_img2 = cv2.normalize(img, None, alpha=0, beta=1.1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        norm_img2 = np.clip(norm_img2, 0, 1)
        norm_img2 = (255*norm_img2).astype(np.uint8)
        img = norm_img2

    elif avg_maxval > 60:
        norm_img2 = cv2.normalize(img, None, alpha=0, beta=0.55, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        norm_img2 = np.clip(norm_img2, 0, 1)
        norm_img2 = (255*norm_img2).astype(np.uint8)
        img = norm_img2
...
img = cv2.imread('/home/nishka/Desktop/CV_Project/deblurred.png')
...
titles = ['Original Image', 'ADAPTIVE', 'BINARY','TRUNC','TOZERO']
images = [img, thresh2, thresh1, thresh3, thresh4]
for i in range (4):
    if i == 1:
